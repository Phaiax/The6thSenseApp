...
public class BluetoothLeUart extends BluetoothGattCallback implements BluetoothAdapter.LeScanCallback {
    ...
    public static final String LOG_TAG = "BluetoothLeUart";
    ...
    public void connectFirstAvailable() {
        Log.d(LOG_TAG,"connectFirstAvailable()");
        ...
        if (newState == BluetoothGatt.STATE_CONNECTED) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                if (!gatt.discoverServices()) {
                    ...
                    Log.d(LOG_TAG, "Could not discover");
                    connectFailure();
                }
            }
            else {
                // Error connecting to device.
                Log.d(LOG_TAG, "Could not connect: " + status);
                connectFailure();
            }
        }
    ...
    public void onServicesDiscovered(BluetoothGatt gatt, int status) {
        ...
        try {
            ...
            Log.e(LOG_TAG, "Could not get rx/tx service, device in use?");
            connectFailure();
            return;
        }
        ...
        for(BluetoothGattDescriptor g : rx.getDescriptors()) {
            Log.d(LOG_TAG, "rx descr: " + g.toString());
        }

        if ( (rx.getProperties() & BluetoothGattCharacteristic.PROPERTY_NOTIFY) != 0) {
            Log.d(LOG_TAG, "rx prop: NORIFY");
        }
        if ( (rx.getProperties() & BluetoothGattCharacteristic.PROPERTY_INDICATE) != 0) {
            Log.d(LOG_TAG, "rx prop: INDICATE");
        }
        if ( (rx.getPermissions() & BluetoothGattCharacteristic.PERMISSION_READ) != 0) {
            Log.d(LOG_TAG, "rx perm: PERMISSION_READ");
        }
        if ( (rx.getPermissions() & BluetoothGattCharacteristic.PERMISSION_READ_ENCRYPTED) != 0) {
            Log.d(LOG_TAG, "rx perm: PERMISSION_READ_ENCRYPTED");
        }
        if ( (rx.getPermissions() & BluetoothGattCharacteristic.PERMISSION_READ_ENCRYPTED_MITM) != 0) {
            Log.d(LOG_TAG, "rx perm: PERMISSION_READ_ENCRYPTED_MITM");
        }
        if ( (rx.getPermissions() & BluetoothGattCharacteristic.PERMISSION_WRITE) != 0) {
            Log.d(LOG_TAG, "rx perm: WRITE");
        }
        if ( (rx.getPermissions() & BluetoothGattCharacteristic.PERMISSION_WRITE_ENCRYPTED) != 0) {
            Log.d(LOG_TAG, "rx perm: PERMISSION_WRITE_ENCRYPTED");
        }
        if ( (rx.getPermissions() & BluetoothGattCharacteristic.PERMISSION_WRITE_ENCRYPTED_MITM) != 0) {
            Log.d(LOG_TAG, "rx perm: PERMISSION_WRITE_ENCRYPTED_MITM");
        }
        if ( (rx.getPermissions() & BluetoothGattCharacteristic.PERMISSION_WRITE_SIGNED) != 0) {
            Log.d(LOG_TAG, "rx perm: PERMISSION_WRITE_SIGNED");
        }
        if ( (rx.getPermissions() & BluetoothGattCharacteristic.PERMISSION_WRITE_SIGNED_MITM) != 0) {
            Log.d(LOG_TAG, "rx perm: PERMISSION_WRITE_SIGNED_MITM");
        }
        ...
        if (!gatt.setCharacteristicNotification(rx, true)) {
            ...
            Log.d(LOG_TAG, "characteristic notification setup failed");
            connectFailure();
            return;
        }
        ...
        if (desc == null) {
            ...
            Log.d(LOG_TAG, "RX characteristic has no client descriptor");
            connectFailure();
            return;
        }
        if ( (desc.getPermissions() & BluetoothGattDescriptor.PERMISSION_READ) != 0) {
            Log.d(LOG_TAG, "desc perm: PERMISSION_READ");
        }
        if ( (desc.getPermissions() & BluetoothGattDescriptor.PERMISSION_READ_ENCRYPTED) != 0) {
            Log.d(LOG_TAG, "desc perm: PERMISSION_READ_ENCRYPTED");
        }
        if ( (desc.getPermissions() & BluetoothGattDescriptor.PERMISSION_READ_ENCRYPTED_MITM) != 0) {
            Log.d(LOG_TAG, "desc perm: PERMISSION_READ_ENCRYPTED_MITM");
        }
        if ( (desc.getPermissions() & BluetoothGattDescriptor.PERMISSION_WRITE) != 0) {
            Log.d(LOG_TAG, "desc perm: WRITE");
        }
        if ( (desc.getPermissions() & BluetoothGattDescriptor.PERMISSION_WRITE_ENCRYPTED) != 0) {
            Log.d(LOG_TAG, "desc perm: PERMISSION_WRITE_ENCRYPTED");
        }
        if ( (desc.getPermissions() & BluetoothGattDescriptor.PERMISSION_WRITE_ENCRYPTED_MITM) != 0) {
            Log.d(LOG_TAG, "desc perm: PERMISSION_WRITE_ENCRYPTED_MITM");
        }
        if ( (desc.getPermissions() & BluetoothGattDescriptor.PERMISSION_WRITE_SIGNED) != 0) {
            Log.d(LOG_TAG, "desc perm: PERMISSION_WRITE_SIGNED");
        }
        if ( (desc.getPermissions() & BluetoothGattDescriptor.PERMISSION_WRITE_SIGNED_MITM) != 0) {
            Log.d(LOG_TAG, "desc perm: PERMISSION_WRITE_SIGNED_MITM");
        }


        if (!desc.setValue(BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE)) {
            Log.d(LOG_TAG, "client descriptor could not be stored locally");
        }
        if (!gatt.setCharacteristicNotification(rx, true)) {
            // Stop if the characteristic notification setup failed.
            Log.d(LOG_TAG, "characteristic notification setup failed (2)");
            connectFailure();
            return;
        }
        if (!gatt.writeDescriptor(desc)) {
            // Stop if the client descriptor could not be written.
            Log.d(LOG_TAG, "client descriptor could not be written");
            connectFailure();
            return;
        }
    ...
    public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
        ...
        Log.d(LOG_TAG, "onDescriptorWrite() with status " + status);
    }
